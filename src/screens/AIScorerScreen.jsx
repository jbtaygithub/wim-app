      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
              ...
              {step.actor && <p className="text-sm text-gray-600">Actor: {step.actor}</p>}

              {Array.isArray(step.tools) && step.tools.length > 0 && (
                <p className="text-sm text-gray-500">Tools: {step.tools.join(', ')}</p>
              )}
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected ? 'ring-2 ring-offset-2 ring-purple-600' : 'hover:bg-gray-300'}
                  `}
                  style={{
        ...
        {aiSuggestion && (
          <div className="w-48 shrink-0 bg-sky-50 rounded-lg p-3 border border-sky-100">
            ...
            <p className="font-bold text-gray-900 capitalize">AI: {aiSuggestion.score}</p>
            <p className="text-xs text-gray-600 italic mt-1">{aiSuggestion.reason}</p>
          </div>
        )}
      </div>
    </div>
    ...
    try {
      ...
      suggestions.forEach((suggestion) => {
        ...
        if (step) {
          ...
          newSuggestions[step.id] = { score: suggestion.score, reason: suggestion.reason }
        }
      })
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            ...
            <p className="text-purple-700">Repetitive, rule-based, high volume, data entry, document processing</p>
          </div>
          <div>
            <span className="font-medium text-purple-800">Medium</span>
            <p className="text-purple-700">Semi-structured, some judgment needed, could use AI assistance</p>
          </div>
          <div>
            <span className="font-medium text-purple-800">Low</span>
            <p className="text-purple-700">Creative, complex judgment, relationship-dependent, strategic</p>
          </div>
        </div>
